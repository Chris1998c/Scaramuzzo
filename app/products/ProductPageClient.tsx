...
const ProductPageClient: FC<Props> = ({ id }) => {
  ...
  useEffect(() => {
    const current = productTranslations[language].products.find(p => p.id === id)
    if (current) {
      setProduct(current)
    } else {
      const otherLanguage = language === 'it' ? 'en' : 'it'
      const matching = productTranslations[otherLanguage].products.find(p => p.id === id)
      if (matching) {
        const corresponding = productTranslations[language].products.find(p =>
          (language === 'it' && p.id === 'shampoo-riflessante-henne' && matching.id === 'reflective-henna-shampoo') ||
          (language === 'it' && p.id === 'maschera-riflessante-henne' && matching.id === 'reflective-henna-mask') ||
          (language === 'en' && p.id === 'reflective-henna-shampoo' && matching.id === 'shampoo-riflessante-henne') ||
          (language === 'en' && p.id === 'reflective-henna-mask' && matching.id === 'maschera-riflessante-henne')
        )
        if (corresponding) setProduct(corresponding)
      }
    }
  }, [id, language])
